...
describe('PageHeader', () => {
  ...
  it('applies cafe theme colors to title', () => {
    const { container } = render(<PageHeader title="Test Title" />);
    
    const h1 = container.querySelector('h1');
    expect(h1).toHaveClass('text-cafe-brown-800');
  });

  it('applies cafe theme colors to subtitle', () => {
    const { container } = render(
      <PageHeader title="Test Title" subtitle="Test subtitle" />
    );
    
    const p = container.querySelector('p');
    expect(p).toHaveClass('text-cafe-brown-600');
  });
  ...
  it('merges custom className with default classes', () => {
    ...
    expect(div).toHaveClass('mb-8', 'text-center', 'custom-class');
  });
